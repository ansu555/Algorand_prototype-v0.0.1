...
// src/AgentWallet.ts
import algosdk2, { Algodv2 as Algodv22 } from "algosdk";
...
var AgentWallet = class _AgentWallet {
  ...
  static async create(userAddress, password, config) {
    ...
    try {
      const response = await axios2.post(`${apiBaseUrl}/agent`, {
        action: "create",
        userAddress,
        password,
        network
      ...
      if (response.data.ok && response.data.mnemonic) {
        const account = algosdk2.mnemonicToSecretKey(response.data.mnemonic);
  ...
  async optIn(assetId) {
    try {
      ...
      const txn = algosdk2.makeAssetTransferTxnWithSuggestedParamsFromObject({
      ...
      const confirmedTxn = await algosdk2.waitForConfirmation(this.algodClient, txId, 4);
      return {
        txId,
  ...
  async transfer(params) {
    try {
      ...
      let txn;
      if (params.assetId === 0) {
        txn = algosdk2.makePaymentTxnWithSuggestedParamsFromObject({
          sender: this.account.addr,
          receiver: params.to,
        ...
        txn = algosdk2.makeAssetTransferTxnWithSuggestedParamsFromObject({
          sender: this.account.addr,
          receiver: params.to,
          amount: BigInt(params.amount),
  ...
  async swap(quote) {
    try {
      const response = await axios2.post(`${this.apiBaseUrl}/swap/prepare`, {
        quote,
        sender: this.account.addr,
        network: this.network
      });
      ...
      const txnBytes = Buffer.from(response.data.txn, "base64");
      const txn = algosdk2.decodeUnsignedTransaction(txnBytes);
      const signedTxn = txn.signTxn(this.account.sk);
      const submitResponse = await axios2.post(`${this.apiBaseUrl}/swap/submit`, {
  ...
  static parseConfig(config) {
    if (typeof config === "string") {
      ...
      const algodClient = network === "mainnet" ? new Algodv22("", "https://mainnet-api.4160.nodely.io", "") : new Algodv22("", "https://testnet-api.4160.nodely.io", "");
      const apiBaseUrl = network === "mainnet" ? "https://10xswap.com/api" : "http://localhost:3000/api";
      return { algodClient, network, apiBaseUrl };
    } else {
      const network = config.network;
      const algodClient = config.algodUrl ? new Algodv22(config.algodToken || "", config.algodUrl, config.algodPort || "") : network === "mainnet" ? new Algodv22("", "https://mainnet-api.4160.nodely.io", "") : new Algodv22("", "https://testnet-api.4160.nodely.io", "");
...
var AutoPilot = class {
  ...
  async createRule(config, ownerAddress) {
    try {
      ...
      const response = await axios3.post(`${this.apiBaseUrl}/rules`, payload);
  ...
  async getRules(ownerAddress) {
    try {
      const response = await axios3.get(`${this.apiBaseUrl}/rules`, {
        params: {
          owner: ownerAddress,
          network: this.network
        }
  ...
  async getRule(ruleId, ownerAddress) {
    try {
      const response = await axios3.get(`${this.apiBaseUrl}/rules/${ruleId}`, {
        params: {
          owner: ownerAddress,
          network: this.network
        }
  ...
  async updateRuleStatus(ruleId, status, ownerAddress) {
    try {
      const response = await axios3.patch(`${this.apiBaseUrl}/rules`, {
        id: ruleId,
        owner: ownerAddress,
        status,
        network: this.network
  ...
  async executeRule(ruleId, ownerAddress) {
    try {
      const response = await axios3.post(`${this.apiBaseUrl}/rules/execute`, {
        id: ruleId,
        owner: ownerAddress,
        network: this.network
      });
  ...
  async deleteRule(ruleId, ownerAddress) {
    try {
      const response = await axios3.delete(`${this.apiBaseUrl}/rules`, {
        params: {
          id: ruleId,
          owner: ownerAddress,
          network: this.network
...
var MarketAnalysis = class _MarketAnalysis {
  ...
  async analyze(request) {
    try {
      const response = await axios4.post(`${this.apiBaseUrl}/mcp/analyze`, {
        coin: request.coin,
  ...
  async getFearGreedIndex() {
    try {
      const response = await axios4.get(`${this.apiBaseUrl}/mcp/fear-greed`);
  ...
  async getTrendingCoins() {
    try {
      const response = await axios4.get(`${this.apiBaseUrl}/mcp/trending`);
  ...
  async getTrendingAlgorandTokens() {
    try {
      const response = await axios4.get(`${this.apiBaseUrl}/mcp/trending/algorand`);
      if (response.data.ok && response.data.trending) {
        return response.data.trending;
      } else {
  ...
  async getNews(coinId, limit = 5) {
    try {
      const response = await axios4.get(`${this.apiBaseUrl}/mcp/news`, {
        params: { coinId, limit }
      });
...
import algosdk3, { Algodv2 as Algodv23 } from "algosdk";
...
var LiquidityPool = class {
  ...
  createDefaultAlgodClient(network) {
    if (network === "mainnet") {
      return new Algodv23("", "https://mainnet-api.4160.nodely.io", "");
    }
    return new Algodv23("", "https://testnet-api.4160.nodely.io", "");
  }
  /**
  ...
  async getPoolInfo() {
    try {
      const response = await axios5.get(`${this.apiBaseUrl}/pools/${this.poolAppId}`, {
        params: { network: this.network }
      });
  ...
  async getSwapQuote(assetIn, amountIn) {
    try {
      const response = await axios5.get(`${this.apiBaseUrl}/pools/${this.poolAppId}/quote`, {
        params: {
          assetIn,
          amountIn,
          network: this.network
  ...
  async createPool(asset1, asset2, feeBps, senderAddress) {
    try {
      const response = await axios5.post(`${this.apiBaseUrl}/pools/create`, {
        asset1,
        asset2,
        feeBps,
      ...
      if (response.data.ok && response.data.txn) {
        const txnBytes = Buffer.from(response.data.txn, "base64");
        return algosdk3.decodeUnsignedTransaction(txnBytes);
      } else {
  ...
  async addLiquidity(params, senderAddress) {
    try {
      const response = await axios5.post(`${this.apiBaseUrl}/pools/${this.poolAppId}/add-liquidity`, {
  ...
  async removeLiquidity(lpTokens, minAmount1, minAmount2, senderAddress) {
    try {
      const response = await axios5.post(`${this.apiBaseUrl}/pools/${this.poolAppId}/remove-liquidity`, {
        lpTokens,
        minAmount1,
        minAmount2,
        sender: senderAddress,
  ...
  async swap(assetIn, amountIn, minAmountOut, senderAddress) {
    try {
      const response = await axios5.post(`${this.apiBaseUrl}/pools/${this.poolAppId}/swap`, {
        assetIn,
        amountIn,
        minAmountOut,
        sender: senderAddress,
